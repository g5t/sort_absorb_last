...
int run_particles_test(_class_particle_soa * particles) {
  ...
  for (long i=0; i<n_particles; ++i){
    for (long j=0; j<14; ++j){
      particles->_doubles[i * 14 + j] = (double)i;
    }
    for (long j=0; j<6; ++j){
      particles->_ints[i * 6 + j] = (int)i;
    }
    particles->_ints[i * 6 + 1] = (int)(i % 3); // absorbed
    for (long j=0; j<7; ++j){
      particles->_randstate[i * 7 + j] = (unsigned long)i;
    }
  ...
  int counter = -3;
  for (long i=0; i<n_particles; i++){
    if (particles->_ints[i*6 + 1] == 0 && particles->_randstate[7*i] == (unsigned long)i){
      long expected = (i % 3) == 0 ? i : (counter+=3) ;
      if (particles->_doubles[14*i] == (double)expected){
        ++correct_count;
      } else {
        printf("Particle %ld: x=%f, _absorbed=%d, rand=%ld, expected %ld\n", i, particles->_doubles[14*i], particles->_ints[6*i+1], particles->_randstate[7*i], expected);
      }
    } else {
      printf("Particle %ld: x=%f, _absorbed=%d, rand=%ld matching%ld\n", i, particles->_doubles[i], particles->_ints[6*i+1], particles->_randstate[7*i], particles->_randstate[7*i] - i);
    }
    if (counter >= n_particles - 3) counter = -3;
  }
